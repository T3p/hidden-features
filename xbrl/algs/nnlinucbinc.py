    ...
    def run(self, horizon: int, throttle: int=100, log_path: str=None) -> None:
        ...
        with tqdm(initial=self.t, total=horizon, postfix=postfix) as pbar:
            while (self.t < horizon):
                ...
                with torch.no_grad():
                    xt = torch.FloatTensor(features[action].reshape(1,-1)).to(self.device)
                ...
                if self.t == self.update_time:
                    ...
                    self.target_model.eval()
                    self.update_time = self.update_time + self.update_every_n_steps #max(1, self.update_time) * 2
